...
async function launchBrowser() {
  if (IS_SERVERLESS) {
    // @sparticuz/chromium only extracts al2/al2023 NSS libs when Lambda-like env is set.
    // Vercel does not set those, so import after setting this or Chromium fails with libnss3.so.
    ...
    // Stale /tmp/chromium from a prior deploy without NSS libs would skip al2/al2023 extract.
    if (existsSync("/tmp/chromium")) {
      const nssOk =
        existsSync("/tmp/al2023/lib/libnss3.so") ||
        existsSync("/tmp/al2/lib/libnss3.so");
      if (!nssOk) {
        await fs.unlink("/tmp/chromium").catch(() => {});
      }
    }
